...
final class SettingTooltipsData {
    ...
    private static void part3(Map<String, String> d) {
        ...
        d.put("ability cooldowns/display", "How the cooldown list is drawn: which abilities it lists, plain text or colour swatch, and how many lines it may use.");
        ...
        d.put("ability cooldowns/mage cooldown reduction", "Shortens every timer by the Mage class cooldown reduction. Only switch it on while you are actually playing Mage in a dungeon.");
        d.put("ability cooldowns/abilities", "Tick the abilities you want timed. The number after each name is that ability's real cooldown in seconds, taken from SkyHanni's table.");
        ...
        d.put("arrow align/min delay", "Cheat build only. Shortest random wait between Aura's frame clicks, 0-1000 ms. Raising it past Max Delay pushes Max Delay up.");
        d.put("arrow align/max delay", "Cheat build only. Longest random wait between Aura's frame clicks, 0-1000 ms. Lowering it below Min Delay pulls Min Delay down.");
        ...
        d.put("dungeon alerts/terracotta timer (f6/m6 boss)", "In the F6/M6 boss (Sadan), shows a countdown over each broken terracotta until it respawns (15s on F6, 12s on M6).");
        d.put("dungeon alerts/sound", "Which sound Secret Sound plays (Experience Orb, Note Pling, Level Up, Anvil Land and more). Click to cycle, then use Play Sound to preview.");
        d.put("dungeon alerts/volume", "Loudness of the secret sound, 0.0-1.0 in 0.1 steps. The sound is played 5 times at once, so it is louder than one normal play.");
        d.put("dungeon alerts/pitch", "Pitch of the secret sound, 0.0-2.0; 1.0 is normal, higher is squeakier.");
        d.put("dungeon alerts/tab list", "Recolours each teammate's name in the tab list by their dungeon class. Needs Class Colors ON.");
        d.put("dungeon alerts/nametags", "Draws a class-coloured tag like [M] Name above each teammate (not you), visible through walls. The normal name tag still shows. Needs Class Colors ON.");
        ...
        d.put("lever aura/range", "Cheat build only. How far away a lever can be, in blocks, and still be flicked by Lever Aura.");
