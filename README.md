...
- Custom Leap Menu — draws 4 big clickable boxes over the real Spirit Leap GUI, one per real teammate, colored by their assigned class; click anywhere in a quadrant instead of the tiny real slot
...
- Sharp Shooter (i4) — always-on sensors for the F7/M7 i4 arrow device (target wall, arrows, hits, aim, held item/helmet, device completion, timed from Storm's death) that log to latest.log for testing. The **cheat** build adds Auto i4: shoots each lit target automatically with Rotate / No Rotate modes and optional next-target predictions (in development)
