...
- Full Block — **cheat build only.** Expands the clickable area of levers, buttons, chests, and Wither Essence blocks so real secrets are easier to click, with a master on/off toggle; buttons also have a Custom Button Size option with Width, Height and Length sliders from normal size to a full block - see [Which jar do I download?](#installing) below.
