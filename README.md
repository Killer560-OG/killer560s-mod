...
- Maxor's Crystals (F7/M7 P1) — Energy Crystal respawn countdown, how long you took to place yours (with a saved personal best), a warning while you're still holding an unplaced crystal, an active-crystals counter, and an optional highlight of the crystals
...
- Posmsg — position-message waypoints: each one draws a ring on the ground and types its own message into party chat ("at hee2" — no coordinates, so teammates need no mod to read it) the moment you walk inside it. Preloaded presets for real rooms (Simon Says/EE2/EE3/Outpour/Recor/Necron's Platform/P5/Mel) plus custom ones you add, each with its own message, radius, ring toggle, colour and an optional "only send once per run"
